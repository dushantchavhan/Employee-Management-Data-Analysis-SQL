...
-- We are planning a cybersecurity project. Find all employees working in the IT 
...
-- The finance team is preparing budgets. List all employees whose salary is greater 
-- than ₹60,000.
SELECT * FROM Employees
WHERE Salary > 60000;

-- We are sending a company-wide newsletter. Find the details of employees whose 
-- email ends with '@example.com'. 
SELECT * FROM Employees
WHERE email = '%@example.com'; 

-- The Marketing department is launching a new campaign and needs a cost-effective 
...
-- A client requires a project lead for their team. Find the employee details where the                  Checkkk
-- name starts with 'S'. 
SELECT * FROM Employees
WHERE FirstName = 'S%';

-- For recruitment analysis, retrieve all employees who joined in February 2024.
SELECT * FROM Employees
WHERE DateOfJoining BETWEEN '2024-01-31' AND '2024-02-29';
...
-- A special campaign is being planned for team leaders. Find employees who have 
-- "Manager" in their name.
SELECT * FROM Employees
WHERE FirstName = 'Manager' AND LastName ='Manager';

-- Department Table ------------------------------------------------------------------------------------------------

-- To identify local opportunities, list all the departments located in Pune. 
SELECT * FROM Departments
WHERE Location = 'Pune';

-- The board is interested in high-budget projects. Find the details of departments 
-- where the budget exceeds ₹10,00,000. 
SELECT * FROM Departments
WHERE AnnualBudget > 1000000;
                          
-- For a leadership meeting, retrieve the departments headed by "Rakesh Mehta". 
SELECT * FROM Departments
WHERE HeadOfDepartment = 'Rakesh Mehta';

-- Marketing expansion is being planned. Find all departments whose names start           Cheackkkkkkk
-- with "M". 
SELECT * FROM Departments
WHERE DepartmentName = "M%";

-- To allocate funds efficiently, list all departments where the budget is between 
...
-- To analyze salary distribution, find the total salary paid in each department.
SELECT DepartmentID, SUM(Salary) AS TotalSalary FROM Employees
GROUP BY DepartmentID;

-- For employee retention analysis, count the number of employees in each department. 
SELECT DepartmentID, COUNT(*) AS TotalEmployees FROM Employees
GROUP BY DepartmentID;

-- The finance team wants to understand salary variation. Find the average salary in each department. 
SELECT DepartmentID, SUM(Salary) AS TotalSalary FROM Employees
GROUP BY DepartmentID;

-- To reward experienced employees, identify the earliest joining date in each department. 
SELECT DepartmentID, MIN(DateOfJoining) FROM Employees
GROUP BY DepartmentID;

-- The HR team is preparing performance reports. Retrieve the maximum salary in each department. 
SELECT DepartmentID, MAX(Salary) AS MaxSalary FROM Employees
GROUP BY DepartmentID;

-- To analyze junior-level hiring, find the minimum salary in each department. 
SELECT DepartmentID, MIN(Salary) AS MinSalary FROM Employees
GROUP BY DepartmentID;

-- The finance team wants to review high-salary employees. Find the total salary paid for employees earning more
-- than ₹60,000 in each department. 
SELECT DepartmentID, SUM(Salary) AS TotalSalary FROM Employees
WHERE Salary > 60000
GROUP BY DepartmentID;

-- To monitor departmental growth, count the number of employees who joined in 2024 in each department. 
SELECT DepartmentID, COUNT(*) AS TotalEmployees FROM Employees
WHERE YEAR(DateOfJoining) = 2024
GROUP BY DepartmentID;

-- For training allocation, count the number of employees with salaries between ₹50,000 and ₹70,000 in each department. 
SELECT DepartmentID, COUNT(*) AS TotalEmployees FROM Employees
WHERE Salary BETWEEN 50000 AND 70000
GROUP BY DepartmentID;

-- For diversity analysis, count the number of employees whose names start with each letter of the alphabet.                    CHECKKKKKK
SELECT DepartmentID, COUNT(*) AS TotalEmployees FROM Employees
GROUP BY DepartmentID;
                             
-- Department Table -----------------------------------------------------------------------------------------------------------------------------------

-- For city-based analysis, count the number of departments located in each city. 
SELECT Location, COUNT(DepartmentName) AS TotalDept FROM Departments
GROUP BY Location;

-- To manage high-budget projects, find the total budget allocated to all departments in each city.
SELECT Location, SUM(AnnualBudget) AS TotaLBudget FROM Departments
GROUP BY Location;
 
-- For leadership tracking, count the number of departments headed by each manager.
SELECT HeadOfDepartment, COUNT(DepartmentName) AS TotalDept FROM Departments
GROUP BY HeadOfDepartment;
 
-- To plan budget allocation, calculate the average budget of departments in each city.
SELECT Location, AVG(AnnualBudget) AS TotalBudget FROM Departments
GROUP BY Location;
 
-- For funding efficiency, find the maximum and minimum budgets among departments in each city.
...
-- To create a full name column, concatenate the first name and last name of each employee.
SELECT FirstName, LastName,
CONCAT(FirstName, ' ', LastName) AS FullName 
FROM Employees;
 
-- For a project report, concatenate the department name and its location to display a complete department address.
 SELECT DepartmentName, Location,
CONCAT(DepartmentName, ' - ', Location) AS DepartmentAddress
FROM Departments;

-- To prepare a personalized email greeting, concatenate "Hello " with the employee's name and a comma. 
SELECT FirstName, LastName,
CONCAT('Hello, ', FirstName, LastName) AS Greeting
FROM Employees;

-- For creating a unique employee ID, concatenate the department ID and employee number. 
SELECT EmployeeID, DepartmentID,
CONCAT(EmployeeID, ' - ', DepartmentID) AS UniqueEmployeeID
FROM Employees;

-- For creating a company contact list, concatenate the employee's phone number with their extension (if applicable).             checkkkkkkkkk


-- To display the complete address, concatenate the street, city, and postal code of each employee. 
SELECT Street, Location, Pincode,
CONCAT(Street, ' ', Location, ' ', Pincode) AS Address
FROM Employees;

-- To create an employee login ID, concatenate the employee’s first name, last name, and their department ID. 
SELECT FirstName, LastName, DepartmentID,
CONCAT(FirstName, ' ', LastName, ' - ', DepartmentID) AS LoginID
FROM Employees;

-- For sending out a personalized notification, concatenate the employee's department with their position. 
SELECT DepartmentName, PositionPlace,
CONCAT( DepartmentName, ' - ', PositionPlace) AS Notification
FROM Employees;

-- To display the employee's work status, concatenate their job title and current project. 
SELECT JobTitle, CurrentProject,
CONCAT(JobTitle, ' - ', CurrentProject) AS WorkStatus
FROM Employees;

-- For an annual report, concatenate the year and employee’s first name to create a unique report reference code. 
SELECT DateOfJoining, FirstName,
CONCAT(DateOfJoining, ' - ', FirstName) AS Report
FROM Employees;
...
-- As part of an organizational review, we need to update the salary of all employees who are part of the IT department (DepartmentID: 3).
...
-- After the recent promotion evaluation, it has been decided that the employee with EmployeeID 101 should be promoted to a "Senior Developer
-- role. Update their job title accordingly and make sure the position is reflected correctly in the employee records. 
UPDATE Employees
SET JobTitle = 'Senior Developer'
WHERE EmployeeID = 101;

-- The HR team has informed us of a change in the contact information of employee with EmployeeID 120. The new contact number for this
-- employee should be updated to "9876543210". Ensure that the correct phone number is reflected in the system for any future communications. 
UPDATE Employees
SET Contact = 9876543210
WHERE EmployeeID = 120;

-- The company has recently implemented a new email domain. All employees who joined after January 1, 2023, need their email addresses
-- updated to reflect the new domain "@newcompany.com". Update the email addresses accordingly for all the affected employees. 
UPDATE Employees
SET Email = "%@newcompany.com"
WHERE DateOfJoining > '2023-01-01';

-- It has come to our attention that there was an error in the naming of the HR department. The department name for DepartmentID 5 needs
-- to be updated from "HR" to "Human Resources" to reflect the company's official title. Perform this update to ensure consistency across the records. 
UPDATE Departments
SET DepartmentName = "Human Resources"
WHERE DepartmentName = "HR";
SET SQL_SAFE_UPDATES = 0;

-- As part of a company-wide salary review process, all employees who are currently earning below ₹50,000 need to receive a salary increase of ₹5,000.
...
-- The Marketing department has recently moved to a new office in a different city. The location of the Marketing department (DepartmentID: 11) needs to
-- be updated to reflect this change. Please update the location to "Mumbai" in the department's records accordingly. 
UPDATE Departments
SET Location = 'Mumbai'
...
-- EmployeeID 110, who was previously working on a different project, has now been assigned to "Project Alpha" after the successful
-- completion of their prior assignment. Please update the project assignment for this employee to reflect this new responsibility. 
UPDATE Employees
SET WorkingProject = "Project Alpha"
WHERE EmployeeID = 110 ;

-- The employee with EmployeeID 103 had a delayed start date due to personal reasons. We need to update their joining date to the correct date,
-- which is 2024-03-15, to ensure the records accurately reflect the employee's actual start date with the company.
UPDATE Employees
...
-- Due to an employee leaving the company, we need to delete the record of EmployeeID 101 from the Employee table. 
SELECT * FROM Employees
WHERE EmployeeID = 101;
DELETE FROM Employees
WHERE EmployeeID = 101;

-- The company is undergoing a restructuring. As part of this process, we need to delete all employees in the HR department
-- (DepartmentID: 2) who joined before 2022. 
SELECT * FROM Employees
WHERE DepartmentID = 2;
DELETE FROM Employees
WHERE DepartmentID = 2 AND DateOfJoining < '2022-01-01';

-- We are cleaning up the database and need to delete employees with a salary lower than ₹30,000, as they are no longer part of the workforce. 
SELECT * FROM Employees;
DELETE FROM Employees
WHERE Salary < 30000;

-- As per a recent audit, employees who joined before 2020 but no longer have any active projects should be removed from the system.
-- Delete these employee records. 
SELECT * FROM Employees;
DELETE FROM Employees
WHERE YEAR(DateOfJoining) < 2020;

-- After the closure of "Project Beta", we need to delete all records of employees who were working on this project
-- (assumed to be recorded in ProjectName). 
SELECT * FROM Employees;
DELETE FROM Employees
WHERE ProjectName = "Project Beta" ;

-- An employee, EmployeeID 135, was added mistakenly and is no longer part of the organization. Please delete their record from the Employee table. 
SELECT * FROM Employees;
DELETE FROM Employees
WHERE EmployeeID = 135 ;

-- The Marketing department (DepartmentID: 3) has undergone significant layoffs. Delete all records of employees who worked in the Marketing
-- department and joined before 2021. 
SELECT * FROM Employees;
DELETE FROM Employees
WHERE DepartmentID = 3 AND YEAR(DateOfJoining) <2021;

-- As part of a data privacy initiative, we need to delete records of employees who have not provided their email addresses or contact numbers. 
SELECT * FROM Employees;
DELETE FROM Employees
WHERE Email = 'null' || Contact = 'null' ;

-- We need to remove all employees whose salary is above ₹1,00,000 but were part of a temporary contract that has now expired. Delete these employee records. 
SELECT * FROM Employees;
DELETE FROM Employees
WHERE Salary = 100000 ;

-- The company has decided to shut down operations in certain regions. Delete the employee records of those who were hired in the "North"
-- region (assuming a Region column exists), and whose employment ended before 2023. 
SELECT * FROM Employees;
DELETE FROM Employees
WHERE Region = 'North' AND YEAR(DateOfJoining) < 2023 ;

-- Situational Questions on Table Schema Modifications (with Retrieval for Data Consistency) -------------------------------------------------------------------

-- Due to a change in company policy, we need to rename the Employee table to Staff. Please perform the necessary operation to rename the table. 
-- Note: Please do retrieve the changes made for consistency of the data. If you are changing the name of the table, make sure to restore
-- it to the original state as Employee if needed.
SELECT * FROM Employees;
ALTER TABLE Employees RENAME TO Stuff;
SELECT * FROM Stuff;
ALTER TABLE Stuff RENAME TO Employees;

-- As the company has decided to start tracking employees' marital status, add a new column MaritalStatus of type VARCHAR(20) to the Employee table. 
...
-- The company has decided to store the employees' date of birth. Add a DateOfBirth column of type DATE to the Employee table. 
-- Note: Please ensure to retrieve the changes made to check the column's impact on future data consistency.
ALTER TABLE Employees
ADD DateOfBirth DATE;
 
-- We need to change the data type of the Salary column in the Employee table from INT to DECIMAL(10,2) to accommodate fractional salary amounts. 
...
-- To better categorize employees, we need to add a new column EmployeeCategory in the Employee table. The column should accept values
...
-- The company wants to keep track of the department name for employees. Therefore, a column DepartmentName should be added to the Employee table. 
...
-- We need to set the default value of the Status column in the Employee table to "Active" for any new employee added to the database.               checkkkk
...
-- As part of a company-wide restructuring, the DepartmentID column in the Employee table should be renamed to TeamID to reflect the new team-based structure. 
...
-- The company has decided to archive old employee data, so the Employee table needs to be partitioned by JoinDate in order to keep check the data for employees who joined before 2020 separate. 
...
-- We need to create a foreign key constraint between the Employee table and the Department table to ensure the integrity of the department data linked to each employee. 
-- Note: Please retrieve the changes made to verify the foreign key relationship, ensuring data integrity and consistency across both tables.
